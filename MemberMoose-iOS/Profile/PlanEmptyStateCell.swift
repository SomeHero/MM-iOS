...
class PlanEmptyStateCell: UITableViewCell {
    ...
    private lazy var createPlanButton: UIButton = {
        ...
        _button.addTarget(self, action: #selector(PlanEmptyStateCell.createPlanClicked(_:)), forControlEvents: .TouchUpInside)
    ...
    private lazy var importPlanButton: UIButton = {
        let _button = UIButton()
        _button.backgroundColor = UIColorTheme.Primary
        _button.setTitleColor(.whiteColor(), forState: .Normal)
        _button.titleLabel?.font = UIFontTheme.Regular(.Small)
        _button.addTarget(self, action: #selector(PlanEmptyStateCell.importPlanClicked(_:)), forControlEvents: .TouchUpInside)
        _button.layer.cornerRadius = 25
        _button.clipsToBounds = true
        _button.setTitle("IMPORT A PLAN", forState: .Normal)
    ...
    override func updateConstraints() {
        ...
        createPlanButton.snp_updateConstraints { (make) in
            make.top.leading.trailing.equalTo(buttonContainer)
            make.height.equalTo(50)
        }
        importPlanButton.snp_updateConstraints { (make) in
            make.top.equalTo(createPlanButton.snp_bottom).offset(10)
    ...
    func createPlanClicked(sender: UIButton) {
        planEmptyStateCellDelegate?.didCreatePlanClicked()
    }
    func importPlanClicked(sender: UIButton) {
